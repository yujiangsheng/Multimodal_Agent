...
> 更新日期: 2025-07 · 版本 0.4.0 · 1042 测试全部通过
...
输出安全守卫在 `_apply_output_guard()` 中对 LLM 生成的回复进行后处理：

| 检查项 | 说明 |
|--------|------|
| PII 脱敏 | 正则检测手机号、邮箱、身份证号等，替换为 `[***]` |
| 内容策略 | 拦截不当内容（暴力、仇恨言论等），返回安全提示 |
| 幻觉标记 | 标记可能存在事实性错误的段落（实验性功能） |

### CircuitBreaker (v0.4.0)

`LLMClient` 内置熔断器机制，防止后端故障时的雪崩效应：

```
正常调用 ──(连续失败 ≥ N)──→ 熔断状态 (拒绝请求)
                                  │
                            (冷却时间到)
                                  │
                                  ▼
...
v0.3.0 引入了 7 个扩展子系统，v0.4.0 进一步加固了可靠性与安全性。所有子系统均集成在编排器中。
