...
[![Tests](https://img.shields.io/badge/Tests-1042%20passed-brightgreen)]()
...
| **输出安全守卫** | OutputGuard PII 脱敏、内容策略拦截、幻觉标记 |
| **LLM 熔断器** | CircuitBreaker 连续失败自动熔断，避免雪崩 |
...
当前状态：**1042 测试，全部通过**（36 个测试文件，>95% 覆盖率）
...
| 场景 | 建议 | 理由 |
|------|------|------|
| 响应慢 | 降低 `num_ctx`（例如 4096） | 减少 KV-cache 内存，加速推理 |
| 简单对话延迟高 | 确保 `FAST_PATH_MAX_LENGTH` 覆盖大多数输入 | 快速路径跳过重型认知阶段 |
| 多模态慢 | 启用 `parallel_modalities=True` | 图像/音频/视频并行预处理 |
| 内存占用高 | 定期调用 `memory.consolidate()` | 清理低价值记忆 |
| 重复误调用 LLM | 启用 `ResponseCache`（默认开启） | LRU+TTL 缓存避免重复计算 |
| GPU 不可用 | 使用 `qwen3-vl:4b` 小模型 | CPU 也可流畅运行 |
...
**Q: 能否用于生产环境？**
A: 当前版本（v0.4.0）适合研究和原型开发。生产部署建议增加身份认证（Web 已支持 API Key）、限流、日志收集等。
...
### v0.4.0 (2025-07)
- **可靠性加固**: CircuitBreaker 熔断器、OutputGuard 输出安全守卫
- **安全增强**: InputGuard Unicode 规范化、RAG 路径遍历防护
- **线程安全**: Session 切换加锁、Team 并行执行细粒度锁
- **行为修正**: Token 计量仅记录最终成功尝试、Graph 前驱失败检查
- **测试覆盖从 876 → 1042 个用例**（新增参数化测试、鲁棒性测试等）
